...
* 运行命令
```shell
docker run -d --restart=always --name sentinel-dashboard-184 \
  -p 8858:8080 \
  -e JAVA_OPTS="-Duser.timezone=Asia/Shanghai -Dfile.encoding=UTF-8" \
  poazy/sentinel-dashboard:1.8.4
```

* 打包命令
```shell
sudo docker buildx build -t poazy/sentinel-dashboard:1.8.4 \
  --build-arg FROM_IMAGE="openjdk:8-jre-alpine" \
  --build-arg APP_NAME="sentinel-dashboard-1.8.4" \
